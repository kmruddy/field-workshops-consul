...
resource "azurerm_virtual_machine" "consul-terraform-sync" {
  ...
  os_profile {
    ...
    custom_data    = base64encode(templatefile("./scripts/consul-tf-sync.sh", { vault_token = var.vault_token, vault_addr = var.vault_addr, consul_server_ip = var.consul_server_ip, bigip_mgmt_addr = var.bigip_mgmt_addr, bigip_admin_user = var.bigip_admin_user, panos_mgmt_addr = var.panos_mgmt_addr, panos_username = var.panos_username, tfc_token = var.tfc_token, tfc_org = var.tfc_org, consul_license = var.consul_license }))
  }
